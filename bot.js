...
const ownerID = "488242693449842700"
// name of the channel to chat in and check for
const globalchat = "uranium-chat" && "✨〡uranium-chat"
...
bot.on("message", (message) => {
  ...
  if (message.channel.name.includes(globalchat)) {
    ...
    setInterval(() => {
      bot.guilds.cache.forEach((guild) => {
        let channel = guild.channels.cache.find(
          (ch) => ch.name.includes(globalchat)
        );
        if (!channel) return;
        messageAttachment =
  ...
  if (message.content.startsWith(prefix + "delete")) {
	...
	messageAttachment =
      ...
      embed1 = new Discord.MessageEmbed()
        .setColor("0x2ECC71")
        .setAuthor(message.author.tag + ` | From ${message.guild.name}`, avicon)
        .setDescription(
          `${message}`
        )
        .setTimestamp()
            ...
            try {
              ...
              if (msg?.embeds[0]?.footer?.text?.includes("Message ID: " + messageID) && msg?.embeds[0]?.author?.name.includes(message.author.tag)) {
                embedNewOut = new Discord.MessageEmbed()
                  ...
                  .setColor("0x2F3136")
                  .setAuthor(message.author.tag, avicon)
                  .setDescription(
                    ``
                  )
                  .setThumbnail(message.guild.iconURL() + `?size=48`);
  ...
  bot.guilds.cache.forEach(async (guild) => {
    if (guild == message.guild) {
      ...
      if (
        ...
        message.content.startsWith("https://imgur.com/") ||
    ...
    if (!channel) return;
    if (!message.channel.name.includes(globalchat)) return;
    if (
      message.content.startsWith("https://media.tenor.com/") ||
      message.content.startsWith("https://media1.tenor.com") ||
      message.content.startsWith("https://media.discord") ||
    ...
    if (
      message.content.startsWith("https://tenor.com/") || message.content.startsWith("https://imgur.com/")
    ) {
      const gifUrl = await getGifFromLink(message.content);
      if (gifUrl) {
    ...
    if (message.content.startsWith("https://tenor.com/" || message.content.startsWith("https://imgur.com/"))) { return; }
    messageAttachment =
      message.attachments.size > 0 ? message.attachments.array()[0].url : null;
